...
public class Buy extends Islev {
	...
	protected int isleviYerineGetir( List<Ev> islemVerisi , List<String> request) {
		...
		if(request.get(0).equals("rent")){
			
			int sira,ev_uygun_degil=0,index=0,control=0;
			...
			String temp[];
			price_offer=Integer.parseInt(request.get(1));
			setList(requestElement, requestNumber, request, 2);
			...
			for(int i=0 ; i<islemVerisi.size() ; i++ )
			{
				...
				if(ev_uygun_degil!=-1)
				{
					if(price_offer>=10*home_price && home_price>maxCost)
					{
						maxCost=home_price;
						index=i;
						control=1;
